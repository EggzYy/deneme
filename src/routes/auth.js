...
router.post('/register', [
  ...
  try {
    ...
    auth.sendTokenResponse(user, 201, res);
  } catch (error) {
    res.status(500).json({
      success: false,
...
router.post('/login', [
  ...
  try {
    ...
    auth.sendTokenResponse(user, 200, res);
  } catch (error) {
    res.status(500).json({
      success: false,
      message: 'Server error',
...
router.put('/profile', auth.protect, async (req, res) => {
  try {
    const allowedUpdates = [
      ...
      'address', 'emergencyContact', 'insurance', 'profilePicture'
    ];
...
// @access  Private
router.post('/logout', auth.protect, (req, res) => {
  res.cookie('jwt', 'none', {
